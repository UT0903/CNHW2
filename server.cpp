            ...
            if(requestP[i].is_connected){ 
                ...
                if(requestP[i].type == LS_REQ){
                    ...
                    fprintf(stderr, "UP_REQ, fd:%d\n", i);
                        requestP[i].fp = fopen(string(svr_path + string(requestP[i].info)).c_str(), "w+");
                        //fprintf(stderr, "open file\n");
                    }
                    
                    MESSAGE msg;
                    if(recv_msg(i, &msg) == FINISH){
                        fprintf(stderr, "UP_REQ FINISH, fd: %d\n", i);
                    ...
                    if(requestP[i].fp == NULL){ // download request
                        ...
                        MESSAGE msg;
                        if((requestP[i].fp = fopen(string(svr_path + string(requestP[i].info)).c_str(), "r")) == NULL){
                            fprintf(stderr, "The ‘%s’ doesn’t exist.\n", requestP[i].info);
                            msg.type = FILE_NOT_FOUND;
                            send_msg(i, msg);
                            init_request(i);
                            continue;
                        }
                    ...
                    if(feof(requestP[i].fp)){
                        send_msg(i, new_FINISH());
                        fprintf(stderr, "DOWN_REQ FINISH, fd: %d\n", i);
                        fclose(requestP[i].fp);
                        init_request(i);
                    }
                    else{
                    ...
                    if(requestP[i].cap == NULL){
                        ...
                        MESSAGE msg;
                        vector<string> req = splitStr2Vec(string(requestP[i].info), ".");
                        if(!file_exist(svr_path + string(requestP[i].info)) || req[req.size()-1] != "mpg"){
                            msg.type = FILE_NOT_FOUND;
                            send_msg(i, msg);
                            init_request(i);
                            continue;
                        ...
                        requestP[i].cap = new VideoCapture(string(svr_path + string(requestP[i].info)).c_str());
                        ...
                        if(peek_recv_msg(i, &peek_msg) == ESC_REQ){ //recv ESC signal
                            ...
                            fprintf(stderr, "PLAY_REQ FINISH, fd: %d\n", i);
                            init_request(i);
                            continue;
                        }
                    }
                    ...
                    init_request(i);
                    fprintf(stderr, "Connection close, fd: %d\n", i);
                }
            }
        }
    }
